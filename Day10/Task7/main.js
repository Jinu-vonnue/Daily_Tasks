/*
 ...
 7.Write a function numberToText(num) that takes an integer and returns its full English text 
representation
...
if (length == 1) {
  ...
  if (num % 100 == 0) {
    ...
    console.log(
      `${ones[Number(numCopy[0]) - 1]} hundred and ${
        tens[Number(numCopy[1]) - 1]
      }`
    );
  } else if (Number(numCopy.slice(1)) > 10 && Number(numCopy.slice(1)) < 20) {
    console.log(
      `${ones[Number(numCopy[0]) - 1]} hundred and ${
        middle[Number(numCopy[2]) - 1]
      }`
    );
  } else if (numCopy[1] == "0") {
    console.log(
      `${ones[Number(numCopy[0]) - 1]} hundred and ${ones[Number(numCopy[2]) - 1]}
      `
    );
  } else {
    console.log(
